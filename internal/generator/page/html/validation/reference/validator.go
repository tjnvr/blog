...
// abspath.Resolver checks local targets, and an optional LocalCheck runs an
// extra check (such as JavaScript syntax) on each existing local file. Image,
// link and script validators differ only by the collaborators passed to
// NewValidator.
package reference
